...
with con:
    ...
    urls = []
    activeappevents = []  #list of active application events
    ...
    allappSQL= "SELECT * FROM appevent"
    ...
    for row in rows:
        ...
        a['start'] = row[3]
        a['end'] = row[4]
        activeappevents.append(a)

    #GET all app events 
    cur.execute(allappSQL)
    rows = cur.fetchall()
    for row in rows:
        a = collections.OrderedDict()
        a['id'] = row[0]
    ...
    d['window']=windows
    d['url']=urls
    d['activeappevents']=activeappevents
    ...
    #WRITE file
    file = 'dropextract.json'
    z = open(file,'w')
    z.writelines("var appData =")
